...
from src.alignment.train.callbacks import FixCLSModelCallback, PissaConvertCallback, SaveProcessorCallback
...
class CLSTrainer(Trainer):
    r"""
    Inherits Trainer to compute cls loss.
    """
    ...
    def save_predictions(self, predict_results: "PredictionOutput") -> None:
        ...
        pred_proba = predict_results.predictions
        label_ids = predict_results.label_ids

        with open(output_prediction_file, "w", encoding="utf-8") as writer:
            res: List[str] = []
            for i in range(len(pred_proba)):
                res.append(json.dumps({"prediction": int(np.argmax(pred_proba[i])), "label": label_ids[i]}))
