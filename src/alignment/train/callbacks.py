...
from transformers import PreTrainedModel, ProcessorMixin, TrainerCallback, AutoModelForSequenceClassification
...
from src.alignment.extras.constants import TRAINER_LOG, V_HEAD_SAFE_WEIGHTS_NAME, V_HEAD_WEIGHTS_NAME, CLS_HEAD_SAFE_WEIGHTS_NAME, CLS_HEAD_WEIGHTS_NAME
from src.alignment.extras.logging import LoggerHandler, get_logger
...
def fix_valuehead_checkpoint(
    ...
    if safe_serialization:
        # the `model.safatensors`` will first saved at the output_dir, and then removed after safe open.
...
def fix_clshead_checkpoint(
    model: "AutoModelForSequenceClassification", output_dir: str, safe_serialization: bool
) -> None:
    r"""
    The model is already unwrapped.
    ...
    for name, param in state_dict.items():
        if name.startswith("base_model.model.score."):
            ...
            decoder_state_dict[name.replace("base_model.model.", "")] = param
    ...
    # os.remove(path_to_checkpoint)
    logger.info("CLS head model saved at: {}".format(output_dir))
