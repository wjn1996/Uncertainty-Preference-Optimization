...
from src.alignment.data.data_utils import Role
...
if TYPE_CHECKING:
    ...
    from src.alignment.data.parser import DatasetAttr
...
def convert_cls(
    ...
    for i in range(len(examples[dataset_attr.prompt])):
        ...
        outputs["prompt"].append(prompt)
        outputs["labels"].append(examples["label"][i])
...
def align_dataset(
    ...
    elif dataset_attr.formatting == "cls":
        convert_func = partial(convert_cls, dataset_attr=dataset_attr, data_args=data_args)
    else:
        convert_func = partial(convert_sharegpt, dataset_attr=dataset_attr, data_args=data_args)

    column_names = list(next(iter(dataset)).keys())
    if dataset_attr.formatting == "cls":
